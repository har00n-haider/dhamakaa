...
/// <summary>
/// Taken from here:
/// https://www.youtube.com/watch?v=R6ScYynIWiU
/// </summary>
public class Slice : MonoBehaviour {
	...
	void Update () {
        ...
        Vector3 o2 = cTrans.position +  cTrans.forward;

        Vector3 o3 = cTrans.position + (cTrans.forward-new Vector3(0f,0.5f,0f)) + cTrans.right*0.2f;
        ...
        if(Physics.Linecast(o1,o2, out hit) // there is definitely better ways for this (checks if the triangle edges intersect with anything)
...

        {
...

                {
...

                    {
...

                        {
                            ...
                            tri.MatchDirection(norm); // flip the triangle if it happens to be the wrong way compared to the original normal
...

            {
...

                {
                    ...
                    var tri = new Triangle() { v1 = intersections[i], v2 = center, v3 = i + 1 == intersections.Count ? intersections[i] : intersections[i + 1] };
            ...
            if(intersections.Count > 0 && Input.GetMouseButtonDown(0)) // intersects and player pressed button, in a realistic use case, you'd like to nest all the above code in this block
